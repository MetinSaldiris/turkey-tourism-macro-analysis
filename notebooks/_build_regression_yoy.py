...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "(KPSS stat 0.19, p ≈ 0.10). ADF rejects the unit root cleanly on "
    "`yoy_arrivals` (p = 0.005) but does not reject on `yoy_real_EUR_TRY` "
    "(p = 0.18). The two tests reverse the null, so the plain reading is: "
...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "  lag 0 for GB with ρ = +0.462 (and at lag −6 for DE with ρ = +0.619 across "
    "  a broad monotonic profile), so using lag 0 would risk same-month "
